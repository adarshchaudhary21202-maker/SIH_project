...
const messageFor = (error) => error.message || 'Unable to complete the request.';
const apiError = (error) => `<p class="error" role="alert">${escapeHtml(messageFor(error))}</p>`;
const empty = (text) => `<div class="empty">${escapeHtml(text)}</div>`;
const field = (name, label, type = 'text', value = '') => `<label class="field">${label}<input name="${name}" type="${type}" value="${escapeHtml(value)}" required /></label>`;
...
function setBusy(button, busy) {
    ...
    (_a = button.dataset).label || (_a.label = button.textContent || 'Submit');
...
function loginPage(error = '') {
    ...
    document.querySelectorAll('[data-role]').forEach(button => button.onclick = () => { selectedRole = button.dataset.role; loginPage(); });
    document.querySelector('#login-form').onsubmit = submitLogin;
    document.querySelector('#create-account').onclick = () => registrationPage();
}
function registrationPage(error = '') {
    ...
    document.querySelector('#register-form').onsubmit = submitRegistration;
    document.querySelector('#back-to-login').onclick = () => loginPage();
}
async function submitRegistration(event) {
    ...
    const password = String(data.get('password') || ''), confirm_password = String(data.get('confirm_password') || ''), role = String(data.get('role') || 'OFFICER');
    if (!full_name || !email || !badge_id || !password || !confirm_password)
        return registrationPage('Complete all required fields.');
    if (password !== confirm_password)
        return registrationPage('Passwords do not match.');
    const button = form.querySelector('button[type="submit"]');
    setBusy(button, true);
    try {
        await api.register({ full_name, email, badge_id, password, role });
        loginPage('Account created. Sign in with your badge ID or email.');
    }
    catch (error) {
        registrationPage(messageFor(error));
    }
    finally {
...
async function submitLogin(event) {
    ...
    if (!badge_id || !password)
        return loginPage('Enter both your email/badge ID and password.');
    const device_id = crypto.randomUUID();
    pendingLogin = { badge_id, password, device_id, device_name: navigator.userAgent.slice(0, 100) };
    const button = form.querySelector('button[type="submit"]');
    setBusy(button, true);
    try {
        const result = await api.login(pendingLogin);
        pendingLogin.badge_id = result.badge_id;
        otpPage(result.development_otp ? `Development OTP: ${result.development_otp}` : 'An OTP has been issued for this account.');
    }
    catch (error) {
        pendingLogin = null;
        loginPage(messageFor(error));
    }
    finally {
        setBusy(button, false);
    }
...
function otpPage(note = '') {
    ...
    document.querySelector('#otp-form').onsubmit = submitOtp;
    document.querySelector('#resend').onclick = resendOtp;
    document.querySelector('#back').onclick = () => { pendingLogin = null; loginPage(); };
    updateResend();
    resendTimer = window.setInterval(updateResend, 1000);
}
function updateResend() { const seconds = Math.max(0, Math.ceil((resendAt - Date.now()) / 1000)); const button = document.querySelector('#resend'), text = document.querySelector('#resend-status'); if (!button || !text)
    return; button.disabled = seconds > 0; text.textContent = seconds ? `Resend available in ${seconds}s.` : 'You can request a new OTP.'; if (!seconds)
    clearInterval(resendTimer); }
async function submitOtp(event) {
    ...
    if (!/^\d{6}$/.test(otp))
        return otpPage('Enter a six-digit OTP.');
    const button = form.querySelector('button');
    setBusy(button, true);
    try {
        const user = await api.establish(await api.verifyOtp(pendingLogin.badge_id, otp, pendingLogin.device_id));
        currentUser = user;
        pendingLogin = null;
        ...
        otpPage(/expired/i.test(text) ? 'This OTP has expired. Request a new code.' : text);
    }
    finally {
        setBusy(button, false);
    }
}
async function resendOtp() { if (!pendingLogin)
    return; const button = document.querySelector('#resend'); setBusy(button, true); try {
    await api.resend(pendingLogin);
    resendAt = Date.now() + 60000;
    updateResend();
    resendTimer = window.setInterval(updateResend, 1000);
    notify('OTP reissued.');
...
function layout(page, content) {
    if (!currentUser)
        return;
    const nav = currentUser.role === 'SUPERVISOR' ? supervisorNav : officerNav;
...
const cards = (items) => `<div class="grid">${items.map(([label, value]) => `<article class="card"><span class="muted">${escapeHtml(label)}</span><strong class="metric">${escapeHtml(value)}</strong></article>`).join('')}</div>`;
async function dashboard() { if (currentUser?.role === 'SUPERVISOR') {
    const [evidence, corrections] = await Promise.all([api.supervisorEvidence(), api.corrections()]);
    layout('dashboard', cards([['Evidence records', String(evidence.length)], ['Pending review', String(evidence.filter(item => item.status === 'PENDING_REVIEW').length)], ['Correction requests', String(corrections.filter(item => item.status === 'PENDING').length)]]));
}
else {
    const [cases, sessions] = await Promise.all([api.cases(), api.sessions()]);
    layout('dashboard', cards([['My cases', String(cases.length)], ['Test sessions', String(sessions.length)], ['Active sessions', String(sessions.filter(item => item.status === 'STARTED').length)]]));
} }
function createCase() { layout('create-case', `<article class="card"><form id="case-form">${field('case_number', 'Case number')}${field('title', 'Case title')}<label class="field">Description<textarea name="description"></textarea></label><button class="primary">Create case</button></form></article>`); document.querySelector('#case-form').onsubmit = async (event) => { event.preventDefault(); try {
    await api.createCase(Object.fromEntries(new FormData(event.currentTarget)));
...
async function verifyEvidence() { if (!selectedEvidence)
    return layout('verify', empty('Choose an evidence record first.')); const result = await api.verifyEvidence(selectedEvidence.id); layout('verify', cards([['Integrity signature', result.hash_valid ? 'Valid' : 'Invalid'], ['Evidence status', result.status]])); }
async function correctionRequests() { const list = await api.corrections(); layout('corrections', list.length ? `<div class="table-wrap"><table class="table"><thead><tr><th>Evidence</th><th>Reason</th><th>Status</th><th>Action</th></tr></thead><tbody>${list.map(item => `<tr><td>${escapeHtml(item.evidence_id)}</td><td>${escapeHtml(item.reason)}</td><td>${escapeHtml(item.status)}</td><td>${item.status === 'PENDING' ? `<button class="secondary" data-correction="${item.id}" data-approved="true">Approve</button> <button class="secondary" data-correction="${item.id}" data-approved="false">Reject</button>` : '—'}</td></tr>`).join('')}</tbody></table></div>` : empty('No correction requests.')); document.querySelectorAll('[data-correction]').forEach(button => button.onclick = async () => { const reason = window.prompt('Review reason (optional):') || ''; try {
    await api.reviewCorrection(button.dataset.correction, button.dataset.approved === 'true', reason);
...
async function custody() { if (!selectedEvidence)
    return layout('custody', empty('Choose an evidence record first.')); if (currentUser?.role === 'OFFICER') {
    layout('custody', `<article class="card"><form id="custody-form">${field('to_user_badge_id', 'Receiving badge ID')}${field('location', 'Transfer location')}${field('reason', 'Reason')}${field('authorization_reference', 'Authorization reference')}<button class="primary">Record transfer</button></form></article>`);
    document.querySelector('#custody-form').onsubmit = async (event) => { event.preventDefault(); try {
        await api.transfer(selectedEvidence.id, Object.fromEntries(new FormData(event.currentTarget)));
        notify('Custody transfer recorded.');
    }
    catch (error) {
        notify(messageFor(error), 'error');
    } };
...
async function logout() { try {
    ...
    location.hash = '';
    loginPage('You have been signed out.');
} }
async function route() {
    ...
    try {
        ...
        currentUser = null;
        return loginPage('Your session has expired. Please sign in again.');
    }
    const requested = location.hash.replace('#/', '') || 'dashboard';
    const allowed = (currentUser.role === 'SUPERVISOR' ? supervisorNav : officerNav).map(item => item[0]);
    const page = allowed.includes(requested) ? requested : 'dashboard';
    if (page !== requested) {
        location.hash = `#/${page}`;
        return;
    }
    const pages = { dashboard, 'create-case': createCase, 'create-session': createSession, kit: kitVerification, capture: testCapture, result: testResult, 'my-evidence': () => evidenceReview(false), review: () => evidenceReview(true), 'evidence-details': evidenceDetails, 'evidence-qr': evidenceQr, verify: verifyEvidence, corrections: correctionRequests, audit: auditTimeline, custody, 'officer-activity': officerActivity };
    try {
        await pages[page]();
    }
    catch (error) {
        layout(page, apiError(error));
    }
}
